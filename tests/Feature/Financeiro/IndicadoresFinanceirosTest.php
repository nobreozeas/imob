...
class IndicadoresFinanceirosTest extends TestCase
{
    use RefreshDatabase, ContratoTestHelpers;

    public function test_dashboard_exibe_indicadores_do_periodo(): void
    {
        ...
        $response = $this->actingAs($user)->get(route('financeiro.dashboard'));

        $response->assertOk();
        $response->assertInertia(fn ($page) => $page
            ->component('Admin/Financeiro/Dashboard')
            ->where('resumo.receitas', 500)
        );
    }

    public function test_usuario_sem_permissao_nao_acessa_dashboard(): void
    {
